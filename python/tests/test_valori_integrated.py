...
try:
    ...
    from valoricore_ffi import ValoricoreEngine
except ImportError:
    try:
        # Try local submodule import (dev mode)
        from valoricore.valoricore_ffi import ValoricoreEngine
    except ImportError as e:
        import traceback
        traceback.print_exc()
        print(f"Could not import ValoricoreEngine: {e}")
        sys.exit(1)
...
def test_integration():
    ...
    print(f"Initializing ValoricoreEngine at {db_path}...")
    engine = ValoricoreEngine(db_path)
