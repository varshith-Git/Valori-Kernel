...
from valoricore.memory import MemoryClient
from valoricore.kinds import NODE_DOCUMENT, NODE_CHUNK
from valoricore.ingest import load_text_from_file
...
def memory_client(monkeypatch):
    ...
    If 'valoricore_ffi' cannot be imported, patches Valoricore to use MockLocalClient.
    """
    use_mock = False
    try:
    ...
    if use_mock:
        ...
        monkeypatch.setattr("valoricore.memory.Valoricore", mock_factory)
...
def test_chunking_semantics():
    from valoricore.chunking import split_by_sentences
