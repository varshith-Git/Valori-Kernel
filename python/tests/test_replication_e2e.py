...
VERIFY_BIN = "./target/debug/valoricore-verify"
PORT = 3000
...
def check_bins():
    if not os.path.exists(NODE_BIN):
        print(f"Node binary not found at {NODE_BIN}. Run `cargo build -p valoricore-node`")
        sys.exit(1)
    if not os.path.exists(VERIFY_BIN):
        print(f"Verify binary not found at {VERIFY_BIN}. Run `cargo build -p valoricore-verify`")
        sys.exit(1)
