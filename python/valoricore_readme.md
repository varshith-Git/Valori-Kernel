...
| `extract_entities(text, namespace=, source=, entity_types=, model=)` | `{"entities", "relationships", ...}` | Extract entity nodes and relationship edges from text via LLM; stores `source`, `source_text_hash`, and relationship `strength` as auditable metadata. Requires `VALORI_EMBED_PROVIDER`. |
