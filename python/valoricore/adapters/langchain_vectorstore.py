...
"""
Complete LangChain VectorStore adapter for Valoricore.
...
from .base import ValoricoreAdapter

logger = logging.getLogger(__name__)


class ValoricoreVectorStore(VectorStore):
    """
    LangChain VectorStore backed by Valoricore's deterministic kernel.
    ...
    Example:
        ...
        >>> from valoricore.adapters import ValoricoreAdapter, ValoricoreVectorStore
        >>> 
        >>> adapter = ValoricoreAdapter(base_url="http://localhost:3000")
        >>> embeddings = OpenAIEmbeddings()
        >>> 
        >>> vectorstore = ValoricoreVectorStore(
        ...     adapter=adapter,
    ...
    def __init__(
        self,
        adapter: ValoricoreAdapter,
        embedding: Embeddings,
    ):
        """
        Initialize Valoricore vector store.
        
        Args:
            adapter: ValoricoreAdapter instance (handles connection & retries)
    ...
    def from_texts(
        ...
        embedding: Embeddings,
        metadatas: Optional[List[dict]] = None,
        adapter: Optional[ValoricoreAdapter] = None,
        **kwargs: Any,
    ) -> "ValoricoreVectorStore":
        """
        Create a Valoricore vector store from texts.
        
        Args:
            texts: List of texts
            embedding: Embeddings model
            metadatas: Optional metadata
            adapter: ValoricoreAdapter instance (required)
            **kwargs: Additional arguments (ignored)
            
        Returns:
            ValoricoreVectorStore instance
        """
        if not adapter:
            raise ValueError("adapter parameter is required for ValoricoreVectorStore.from_texts()")
...
if __name__ == "__main__":
    print("ValoricoreVectorStore - LangChain Integration")
    print("=" * 50)
    print("\nExample:")
    print("""
from langchain_openai import OpenAIEmbeddings
...
# Add documents
texts = [
    "Valoricore is a deterministic vector database",
...
# Search
results = vectorstore.similarity_search("What is Valoricore?", k=2)
