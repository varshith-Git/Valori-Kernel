...
logger = logging.getLogger("valoricore.adapter")
...
class ValoricoreAdapter:
    def __init__(
        ...
        if not api_key and not dev_mode:
            logger.warning("ValoricoreAdapter initialized without API Key in production mode (dev_mode=False). Requests may fail.")
    ...
    def upsert_vector(
        ...
        Returns:
            memory_id assigned by Valoricore
        """
