...
| S3 — Shard-routing infrastructure: deterministic `shard_for_namespace()`, `DataPlaneState` multi-shard aware; NOT yet wired into handlers — discovered a separate pre-existing bug (`Auto*` writes always land in namespace 0) that blocks it, ~60-call-site fix scoped as S3a | [phase-S3-shard-routing-infrastructure.md](phase-S3-shard-routing-infrastructure.md) | `Node-scaleup` | ✅ done (infra only) |
