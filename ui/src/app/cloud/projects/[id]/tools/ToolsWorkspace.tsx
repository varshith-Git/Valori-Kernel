...
export function ToolsWorkspace({
    ...
    const { dim, online, index } = useHealth(projectId)
    ...
    const changeCollection = useCallback(
        (name: string) => {
            setNamespace(name)
            // Shallow — keeps the collection choice bookmarkable/shareable
            // without a full navigation or losing tab state.
    ...
    const deleteRecord = async (id: number) => {
        const res = await fetch(`/api/cloud/projects/${projectId}/delete`, {
            method: 'POST',
        ...
        <div className="flex flex-col gap-5 w-full">
            ...
            <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as string)}>
                ...
                <TabsContent value="search" className="mt-5">
                    <MultiSearch projectId={projectId} namespace={namespace} dim={dim} onDelete={deleteRecord} />
                </TabsContent>
                <TabsContent value="upload" className="mt-5">
                    <DocumentUploadTab projectId={projectId} namespace={namespace} onAskQuestion={handleAskQuestion} />
                </TabsContent>
                <TabsContent value="bulk" className="mt-5">
                    <BulkInsertTab projectId={projectId} namespace={namespace} dim={dim} />
                </TabsContent>
                <TabsContent value="visualize" className="mt-5">
                    <VisualizeTab projectId={projectId} namespace={namespace} dim={dim} />
                </TabsContent>
                <TabsContent value="ask" className="mt-5">
                    <AskTab projectId={projectId} namespace={namespace} initialQuestion={pendingQuestion} />
                </TabsContent>
                <TabsContent value="docs" className="mt-5">
                    <DocumentsTab projectId={projectId} namespace={namespace} />
                </TabsContent>
                <TabsContent value="treerag" className="mt-5">
                    <TreeRagTab projectId={projectId} namespace={namespace} />
                </TabsContent>
                <TabsContent value="community" className="mt-5">
                    <CommunityTab projectId={projectId} namespace={namespace} />
                </TabsContent>
                <TabsContent value="entities" className="mt-5">
                    <EntityExtractionTab projectId={projectId} namespace={namespace} />
                </TabsContent>
                <TabsContent value="eval" className="mt-5">
                    <EvalTab projectId={projectId} namespace={namespace} />
                </TabsContent>
                <TabsContent value="diff" className="mt-5">
                    <DiffTab projectId={projectId} namespace={namespace} />
                </TabsContent>
                <TabsContent value="contradict" className="mt-5">
                    <ContradictionTab projectId={projectId} namespace={namespace} />
                </TabsContent>
                <TabsContent value="verify" className="mt-5">
                    <VerifyTab projectId={projectId} namespace={namespace} />
                </TabsContent>
                <TabsContent value="certify" className="mt-5">
                    <CertifyTab projectId={projectId} namespace={namespace} collection={namespace} />
                </TabsContent>
                <TabsContent value="gdpr" className="mt-5">
                    <GdprTab projectId={projectId} namespace={namespace} />
                </TabsContent>
                <TabsContent value="compliance" className="mt-5">
                    <CompliancePackTab projectId={projectId} namespace={namespace} collection={namespace} />
                </TabsContent>
                <TabsContent value="info" className="mt-5">
                    <CollectionInfo projectName={projectName} namespace={namespace} dim={dim} />
                </TabsContent>
            </Tabs>
        </div>
    )
