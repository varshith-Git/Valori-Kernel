...
async function fetchGlobalStateHash(): Promise<string | null> {
  try {
    const res = await fetchWithTimeout(`${getApiUrl()}/v1/proof/state`, { headers: nodeHeaders(), cache: "no-store" });
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (record_id !== undefined) {
      const metaRes = await fetchWithTimeout(`${getApiUrl()}/v1/memory/meta/get?target_id=record:${record_id}`, { headers: nodeHeaders() });
    ...
    if (retrievalMode === "graph" && query_vector && record_id === undefined) {
      try {
        const graphRes = await fetchWithTimeout(`${getApiUrl()}/v1/graphrag`, {
          method: "POST",
    ...
    if (retrievalMode === "graph" && !nativeGraphUsed && rankedResults.length > 0) {
      ...
      if (docEdges.size > 0) {
        ...
        try {
          const nodesRes = await fetchWithTimeout(`${getApiUrl()}/graph/nodes?collection=${collection}`, { headers: nodeHeaders() });
          if (nodesRes.ok) {
        ...
        for (const hit of expandable) {
          ...
          for (const rid of docRecordIds) {
            ...
            try {
              const mr = await fetchWithTimeout(`${getApiUrl()}/v1/memory/meta/get?target_id=record:${rid}`, { headers: nodeHeaders() });
              if (!mr.ok) continue;
    ...
    for (const r of rankedResults.slice(0, 5)) {
      ...
      try {
        const sgRes = await fetchWithTimeout(`${getApiUrl()}/graph/subgraph?root=${chunkNodeId}&depth=1`, { headers: nodeHeaders() });
        ...
        for (const node of sg.nodes) {
          ...
          if (node.kind === 1) {
            try {
              const metaRes = await fetchWithTimeout(`${getApiUrl()}/v1/memory/meta/get?target_id=node:${node.id}`, { headers: nodeHeaders() });
              if (metaRes.ok) {
