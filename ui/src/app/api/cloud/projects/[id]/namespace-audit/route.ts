...
// GET /api/cloud/projects/[id]/namespace-audit?namespace=...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [nodesRes, timelineRes, proofRes] = await Promise.allSettled([
    fetchWithTimeout(`${nodeUrl}/graph/nodes?collection=${encodeURIComponent(namespace)}`, { headers: JSON_HEADERS }),
    fetchWithTimeout(`${nodeUrl}/timeline?limit=2000`, { headers: JSON_HEADERS }),
    fetchWithTimeout(`${nodeUrl}/v1/proof/event-log`, { headers: JSON_HEADERS }),
  ]);
  ...
  for (let i = 0; i < rawEvents.length; i++) {
    ...
    const touches =
      recordIds.some((rid) => nsRecordIds.has(rid)) || nodeIds.some((nid) => nsNodeIds.has(nid));
    if (touches) {
      nsEvents.push({
        event_id: i,
        raw,
  ...
  const nsProofHash = crypto.createHash("sha256").update(nsEventIds.join(",")).digest("hex");

  // -- Global proof -------------------------------------------------------------
  type ProofResp = { final_state_hash?: string; event_log_hash?: string; event_count?: number };
  const proof: ProofResp =
    proofRes.status === "fulfilled" && proofRes.value.ok ? await proofRes.value.json().catch(() => ({})) : {};
