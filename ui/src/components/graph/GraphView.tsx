...
export function GraphView({ projectId, namespace }: { projectId: string; namespace: string }) {
  const { nodes, docNodes, chunkNodes, isLoading } = useGraph(namespace, projectId);
  ...
  const { edges: selectedEdges } = useNodeEdges(selectedNode?.node_id ?? null, projectId);
