...
async function fetchVectorsBatched(
  ...
  for (let i = 0; i < ids.length; i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.all(
      batch.map((id) =>
        fetch(`${projectId ? `/api/cloud/projects/${projectId}/records/${id}${qs}` : `/api/records/${id}${qs}`}`)
...
export function VisualizeTab({ projectId, namespace, dim }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const nsRes = await fetch(`${projectId ? `/api/cloud/projects/${projectId}/namespaces` : `/api/namespaces`}`).catch(() => null);
      if (nsRes?.ok) {
