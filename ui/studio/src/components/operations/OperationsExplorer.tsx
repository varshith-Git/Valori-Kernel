...
function RowMenu({ op, operationHref }: { op: OperationSummary; operationHref: (opId: string) => string }) {
...
export function OperationsExplorer({
  ...
  projectId: string;
  /** Builds the "view details" URL for one operation — host-owned routing
   *  (e.g. `/cloud/projects/{id}/operations/{opId}` vs. `/operations/{opId}`). */
  ...
  const fetchOperations = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(transport.path(projectId, "/operations"));
      if (res.status === 503) {
      ...
      <PageHeader
        title={
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl font-bold tracking-tight text-foreground leading-none">Operations</span>
          </div>
        }
        subtitle="Monitor real-time kernel execution, query receipts, and cryptographic verification trails."
        actions={
          <div className="flex items-center gap-3">
            {/* Live badge */}
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-center pr-2">
                  <RowMenu op={op} operationHref={operationHref} />
                </div>
              </a>
            );
          })}
