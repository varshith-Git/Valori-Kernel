...
// Ported from valori-kernel/ui's components/operations/OperationDetailView.tsx
// (a dual-mode component already shared by Desktop Local and Desktop Cloud
// via a `projectId ?` ternary) and valori-ui's near-identical separately
// maintained copy. The investigation found only route-string/projectId
// differences between the two — this is the shared component both were
// already converging on, using `useTransport()` instead of the ternaries
// and host-supplied `backHref` instead of a hardcoded route.
//
// The "Execution Explorer" tab is intentionally NOT ported wholesale: its
// graph view depends on `@xyflow/react`, a dependency Studio does not
// currently declare (see the Phase G1 report — introducing it wasn't
// authorized in this phase). Instead this component fetches the execution
// record itself (same endpoint pattern as everything else) and exposes it
// through an optional `renderExecution` render-prop; a host that already
// depends on `@xyflow/react` (both Kernel apps do) supplies its own
// ExecutionExplorer there. Omitting the prop simply drops that one tab —
// Overview/Results/Proof/Metrics are unaffected either way.
...
export function OperationDetailView({
  ...
  projectId: string;
  operationId: string;
  /** Host-owned "back to operations list" URL — rendered as a plain link,
   *  not a router push, so this component never assumes a Next.js router. */
  backHref: string;
  /** Optional host-supplied Execution Explorer tab content (see module
   *  comment) — receives the raw `GET .../operations/:id/execution` body
   *  and the in-flight loading state. Omit to hide that tab entirely. */
  renderExecution?: (data: unknown, loading: boolean) => React.ReactNode;
}) {
  const transport = useTransport();
  const isIngestOperation = operationId.startsWith("ingest-");
  ...
  const fetchDetail = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(transport.path(projectId, `/operations/${encodeURIComponent(operationId)}`));
      if (!res.ok) {
  ...
  const fetchExecution = async () => {
    ...
    try {
      const res = await fetch(transport.path(projectId, `/operations/${encodeURIComponent(operationId)}/execution`));
      const data = await res.json();
      setExecutionData(data);
    } catch {
      // Execution trace is best-effort — the rest of the view still works.
    } finally {
  ...
  // WAL journal entry backs them.
  if (isIngestOperation) {
    return (
      <div className="flex flex-col gap-6 w-full max-w-[1400px]">
        <div className="flex items-center justify-between">
          <a href={backHref} className="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-foreground -ml-2 px-2 py-1.5">
            <ArrowLeft className="h-4 w-4" />
            Back to Operations
          </a>
          <Button variant="outline" size="sm" onClick={fetchExecution} className="gap-2 border-border/80 text-muted-foreground hover:text-foreground">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-border/80 bg-gradient-to-br from-card via-card/90 to-[var(--v-accent-muted)]/30 p-6 shadow-sm">
          <div className="flex items-center gap-2.5 flex-wrap">
            <span className="font-mono text-xl font-bold text-foreground">{operationId}</span>
          </div>
        ...
        <div className="rounded-2xl border border-border/80 bg-card/60 p-6 shadow-sm min-h-[360px]">
          {renderExecution ? (
            renderExecution(executionData, loadingExecution)
          ) : (
            <p className="text-sm text-muted-foreground">Execution trace not available.</p>
          )}
        </div>
      </div>
    );
  ...
  const tabs = [
    ...
    ...(renderExecution ? [{ id: "execution" as const, label: "Execution Explorer", icon: Network }] : []),
    { id: "results" as const, label: "Results", icon: FileCode },
    { id: "proof" as const, label: "Proof & Receipt", icon: ShieldCheck },
    { id: "metrics" as const, label: "Metrics", icon: Cpu },
  ];
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-[1400px]">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={fetchDetail} className="gap-2 border-border/80 text-muted-foreground hover:text-foreground">
          <RefreshCw className="h-3.5 w-3.5" />
          Refresh
        </Button>
      </div>
      ...
      <div className="relative overflow-hidden rounded-2xl border border-border/80 bg-gradient-to-br from-card via-card/90 to-[var(--v-accent-muted)]/30 p-6 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                <StatusBadge tone="success">{op.status.toUpperCase()}</StatusBadge>
          ...
          <div className="flex flex-col md:items-end gap-1 text-xs text-muted-foreground font-mono bg-background/50 p-3 rounded-xl border border-border/60">
            ...
            <div className="text-[10px] text-muted-foreground/60">UNIX EPOCH: {op.timestamp_unix}</div>
          </div>
        </div>
      </div>
        ...
        {activeTab === "overview" && (
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between border-b border-border/60 pb-4">
              ...
              <CopyBtn text={JSON.stringify(op.overview, null, 2)} label="Copy JSON" />
            </div>
        ...
        {activeTab === "results" && (
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between border-b border-border/60 pb-4">
              ...
              <CopyBtn text={JSON.stringify(op.results, null, 2)} label="Copy JSON" />
            </div>
        ...
        {activeTab === "proof" && (
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between border-b border-border/60 pb-4">
              ...
              <CopyBtn text={JSON.stringify(op.proof, null, 2)} label="Copy JSON" />
            </div>
            ...
            <div className="flex flex-col gap-4 font-mono text-xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="rounded-xl border border-border/60 bg-background/80 p-4 flex flex-col gap-1.5">
                  <span className="text-muted-foreground font-sans font-semibold text-xs uppercase tracking-wider">State Hash (Before)</span>
        ...
        {activeTab === "metrics" && (
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between border-b border-border/60 pb-4">
              ...
              <CopyBtn text={JSON.stringify(op.metrics, null, 2)} label="Copy JSON" />
            </div>
