...
// -- Main page -----------------------------------------------------------------
// Mounted by every host under a project's Metrics screen. The WAL/snapshot
// "local files" info cards need the browser and node to share a
// filesystem, which is only ever true for a Local runtime — gated by
// `capabilities.localFilesystem`, supplied by the host via <StudioProvider>
// (see runtime/capabilities.ts). This component never infers its runtime
// from `projectId` itself — `projectId` is opaque (see runtime/project.ts).
export function MetricsView({
  ...
  const localFilesystem = capabilities.localFilesystem;
  // -- Time series state ------------------------------------------------------
  ...
  const poll = useCallback(async () => {
    try {
      const pingUrl = transport.path(projectId, "/metrics/ping");
  ...
  const { data: snapsData } = useSWR<{ snapshots: SnapshotEntry[]; disabled?: boolean }>(
    projectId ? transport.path(projectId, `/storage/snapshots`) : "/api/storage/snapshots",
    fetcher,
  ...
  const { data: filesData } = useSWR<{ files: LocalFile[] }>(
    localFilesystem ? "/api/local-files" : null,
    fetcher,
