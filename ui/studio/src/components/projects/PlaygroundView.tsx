...
// Mounted by every host under a project's Playground screen. Whether the
// collection picker is shown (vs. a fixed "default" collection) is a runtime
// capability, not something inferred from `projectId` (opaque — see
// runtime/project.ts) — see `capabilities.multiCollectionPicker`.
...
export function PlaygroundView({
  ...
  // One "default" collection when the runtime says there's nothing to
  // switch between; otherwise pull the project's real collection list and
  // let the picker switch among them.
  const multiCollectionPicker = capabilities.multiCollectionPicker;
  const { raw: allCollections } = useCollections(projectId);
  // `collection` (state, below) is the canonical display name — it drives
  // the picker and is what the user sees. But requests go straight to the
  // node's raw HTTP API, which only knows raw namespaces (a Local host may
  // have a legacy-prefixed rawNamespace behind a bare canonical name — see
  // useCollections's own docs) — so every substitution into the sample
  // body/path below must resolve through `rawCollection`, never `collection`
  // itself.
  const collections = multiCollectionPicker && allCollections.length > 0 ? allCollections : [{ name: "default", rawNamespace: "default" }];
  const [collection, setCollection] = useState("default");
  const rawCollection = collections.find((c) => c.name === collection)?.rawNamespace ?? collection;
  ...
  useEffect(() => {
    if (dim != null && isSample && selected.sampleBody) {
      setBody(JSON.stringify(selected.sampleBody(dim, rawCollection), null, 2));
    }
  ...
  useEffect(() => {
    setPath((p) => withCollection(p, rawCollection));
    if (isSample && selected.sampleBody) {
      setBody(JSON.stringify(selected.sampleBody(dim ?? 8, rawCollection), null, 2));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [collection]);
  ...
  const pick = (ep: EndpointDef) => {
    setSelected(ep);
    setMethod(ep.method);
    setPath(withCollection(ep.path, rawCollection));
    setBody(ep.sampleBody ? JSON.stringify(ep.sampleBody(dim ?? 8, rawCollection), null, 2) : "");
    setIsSample(true);
    setResult(null);
    setError(null);
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch(transport.path(projectId, `/playground`), {
        method: "POST",
  ...
  return (
    <div className="max-w-[1400px] flex flex-col gap-5 pb-6">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-3 shrink-0 pt-0.5">
            {/* A "Docs" link lived here before extraction, pointing at a
                Desktop-Local-only /help route — dropped rather than made
                host-configurable for one static link; Cloud's own
                PlaygroundView never had it either. */}
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden flex">
        {/* Endpoint catalog */}
        <div className="w-60 shrink-0 flex flex-col gap-0 border-r border-border overflow-y-auto">
          {CATALOG.map((group) => (
            <div key={group.label} className="px-3 py-3 border-b border-border last:border-b-0">
              <div className="flex items-center gap-1.5 mb-1.5">
                <group.icon size={11} className="text-muted-foreground/60 shrink-0" />
                <p className="text-[10px] uppercase tracking-wider font-semibold text-muted-foreground/70">{group.label}</p>
              </div>
              <div className="flex flex-col">
                {group.endpoints.map((ep) => (
                  <button
                    ...
                    onClick={() => pick(ep)}
                    className={`flex items-center gap-2 text-left px-2 py-1.5 rounded-lg text-sm transition-colors ${
                      selected === ep
                  ...
                  >
                    ...
                    <span className="truncate text-[13px]">{ep.name}</span>
                  </button>
                ))}
              </div>
            </div>
