...
// -- Main GraphView --------------------------------------------------------------
// Ported from valori-kernel/ui's GraphTab (was one of many tabs within a
// collection view there) — here it's a standalone per-project page, and
// `namespace` is hardcoded to the default namespace since the cloud
// dashboard doesn't have collection browsing/switching yet (see follow-ups).
...
export function GraphView({
  ...
  projectId: string;
  namespace: string;
  /** True when mounted inside a host's own tab/panel chrome (e.g. a
   *  collection workspace's Graph tab) rather than as this component's own
   *  full page. Hides the node/doc/chunk count summary line, which is
   *  redundant with a host's own collection-level header in that context —
   *  graph rendering, the Tree/Canvas toggle, and all loading/error/data
   *  behavior are unchanged either way. Defaults to false so every existing
   *  caller (today, always a standalone page) is unaffected. */
  embedded?: boolean;
}) {
  const transport = useTransport();
  const { nodes, docNodes, chunkNodes, isLoading } = useGraph(projectId, namespace);
  ...
  useEffect(() => {
    ...
    async function loadEdges() {
      ...
      const workers = Array.from({ length: Math.min(CONCURRENCY, queue.length) }, async () => {
        ...
        while ((d = queue.shift())) {
          if (cancelled) return;
          const r = await fetch(transport.path(projectId, `/graph/edges/${d.node_id}`));
          if (cancelled) return;
  ...
  const { edges: selectedEdges } = useNodeEdges(projectId, selectedNode?.node_id ?? null);
