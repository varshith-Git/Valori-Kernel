...
async function runScan(opts: ScanOpts): Promise<{ scanned: number; skipped: number }> {
  const {
    transport, projectId, namespace, scanLimit, strengthThreshold, signal,
    onProgress, onPair,
  ...
  const auditRes = await fetch(
    transport.path(projectId, `/namespace-audit?namespace=${encodeURIComponent(namespace)}`),
  ...
  const metaFetches = ids.map((id) =>
    fetch(transport.path(projectId, `/meta?target_id=record:${id}`), { cache: "no-store", signal })
  ...
  for (let i = 0; i < withText.length; i++) {
    ...
    try {
      const searchRes = await fetch(transport.path(projectId, `/search`), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    for (const hit of searchResults) {
      ...
      if (!bMeta) {
        ...
        try {
          const mr = await fetch(transport.path(projectId, `/meta?target_id=record:${hit.id}`), { cache: "no-store", signal });
