...
interface Props {
  projectId: string;
  namespace: string;
  /** Where "Configure in Settings" points — host-specific. */
  settingsHref?: string;
}

export function CommunityTab({ projectId, namespace, settingsHref = "/settings" }: Props) {
  ...
  async function runDetect() {
    ...
    setDetectError(null);
    try {
      const res = await fetch(transport.path(projectId, `/community?action=detect`), {
        method: "POST",
  ...
  async function runSearch() {
    ...
    try {
      ...
      const res = await fetch(transport.path(projectId, `/community?action=search`), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <section className="flex flex-col gap-3 border-t border-border pt-6">
        ...
        {!hasEmbed && (
          <p className="text-xs text-amber-500 rounded border border-amber-800/30 bg-amber-950/20 px-3 py-2">
            ...
            <a href={settingsHref} className="underline">Configure in Settings →</a>
          </p>
        )}
