...
function DocumentCard({
  ...
  const loadChunks = useCallback(async () => {
    ...
    try {
      ...
      const edgesRes = await fetch(transport.path(projectId, `/graph/edges/${nodeId}`));
      ...
      const nodesRes = await fetch(transport.path(projectId, `/graph/nodes?collection=${encodeURIComponent(namespace)}`));
      ...
      await Promise.all(
        edgeList.slice(0, 100).map(async (e) => {
          ...
          const mr = await fetch(transport.path(projectId, `/meta?target_id=record:${rid}`));
          if (!mr.ok) return;
  ...
  const handleUpdateFile = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch(transport.path(projectId, `/ingest/update`), { method: "POST", body: form });
...
export function DocumentsTab({ projectId, namespace }: { projectId: string; namespace: string }) {
  ...
  const swrKey = transport.path(projectId, `/graph/nodes?collection=${encodeURIComponent(namespace)}&kind=0&limit=${visibleCount}`);
  ...
  useEffect(() => {
    ...
    Promise.all(
      missing.map(async (n) => {
        const res = await fetch(transport.path(projectId, `/meta?target_id=document:${n.node_id}`));
