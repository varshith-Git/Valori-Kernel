...
async function buildPack(transport: Transport, collection: string, namespace: string, projectId: string): Promise<CompliancePack> {
  // Attestation + live state
  const [auditRes, healthRes] = await Promise.all([
    fetch(transport.path(projectId, `/namespace-audit?namespace=${encodeURIComponent(namespace)}`), { cache: "no-store" }),
    fetch(transport.path(projectId, `/health`), { cache: "no-store" }).catch(() => null),
  ]);
...
export function CompliancePackTab({
  ...
  const generate = useCallback(async () => {
    ...
    setError(null);
    try {
      setPack(await buildPack(transport, collection, namespace, projectId));
    } catch (e) {
