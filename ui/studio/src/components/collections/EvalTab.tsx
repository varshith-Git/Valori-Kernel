...
async function runEvaluation(
  ...
  for (const pair of pairs) {
    ...
    try {
      ...
      anyVec ??= qVec;
      const retrieved = await searchVec(transport, qVec, k, namespace, pair.question, projectId);
      ...
      const oracleResults = await searchVec(transport, aVec, oracleK, namespace, undefined, projectId);
  ...
  if (anyVec) {
    try {
      ...
      const all = await searchVec(transport, zeroVec, 10_000, namespace, undefined, projectId);
  ...
  const startEval = useCallback(async () => {
    ...
    try {
      const result = await runEvaluation(
        transport, pairs, k, namespace, embedCfg,
