...
// adapted for multi-tenancy. Studio S9 reconciliation: the "client pipeline"
// this tab can drive is NOT client-side JS embedding — it's a *host* Next.js
// server route (e.g. Local's app/api/ingest/route.ts) that chunks + embeds +
// inserts using credentials the user typed into Settings, as a fallback for
// nodes with no server-side embed provider configured. That server route
// stays entirely host-side (Studio must never contain Next.js server code);
// this component's only job, gated by `capabilities.clientEmbeddingFallback`,
// is to additionally submit the user's stored embedding config as extra
// form fields on the same POST — the host's own ingest route decides what
// to do with them. When the capability is off (the default — unchanged
// Cloud behavior), this component drives only the server-pipeline / Tree-RAG
...
interface Props {
  ...
  /** Builds the "view execution" URL for an ingest operation — host-owned
   *  routing. Omit to hide the link. */
  operationHref?: (opId: string) => string;
  /** Gates the client-embedding-fallback form fields (§ module comment).
   *  Omit or set clientEmbeddingFallback: false to keep exactly the
   *  server-pipeline-only behavior this component has always had. */
  capabilities?: StudioCapabilities;
  /** Host-owned link to wherever the user configures their stored
...
export function DocumentUploadTab({ projectId, namespace, onAskQuestion, operationHref, capabilities, settingsHref }: Props) {
  ...
  const clientFallback = capabilities?.clientEmbeddingFallback ?? false;
  ...
  useEffect(() => {
    fetch(transport.path(projectId, `/health`))
  ...
  const ingest = async () => {
    ...
    if (chunkMode === "tree") {
      try {
        ...
        const res = await fetch(transport.path(projectId, `/tree/build`), {
          method: "POST",
          body: form,
        });
    ...
    if (clientFallback) {
      // Extra fields for a host ingest route that can fall back to its own
      // chunk+embed+insert pipeline using the user's stored credentials —
      // ignored by a route that only drives the node's server-side pipeline.
      form.append("provider", embedCfg.provider);
      form.append("model", embedCfg.model);
      form.append("apiKey", embedCfg.apiKey);
      form.append("endpoint", embedCfg.endpoint);
      form.append("chunkSize", String(embedCfg.chunkSize));
      form.append("chunkOverlap", String(embedCfg.chunkOverlap));
    ...
    try {
      ...
      const res = await fetch(transport.path(projectId, `/ingest`), { method: "POST", body: form });
  ...
  const providerReady = clientFallback
      ...
      {clientFallback && (
        <div className="flex items-center justify-between rounded-lg border border-border bg-card px-4 py-3">
          <div className="flex items-center gap-3 text-xs">
            ...
            <span className="font-mono text-accent-foreground">
              {embedCfg.chunkSize}/{embedCfg.chunkOverlap}
            </span>
            {!providerReady && (
          ...
          {settingsHref && (
            <a href={settingsHref} className="text-xs text-muted-foreground hover:text-accent-foreground transition-colors">
              configure →
            </a>
          )}
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-3 shrink-0">
              {result.operation_id && operationHref && (
                <a
                  href={operationHref(result.operation_id)}
