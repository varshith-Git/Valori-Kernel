...
async function fetchSnapshot(transport: Transport, namespace: string, projectId: string): Promise<NsSnapshot> {
...
export function DiffTab({ projectId, namespace }: { projectId: string; namespace: string }) {
  ...
  // Load namespace list
  useEffect(() => {
    fetch(transport.path(projectId, `/namespaces`), { cache: "no-store" })
      .then((r) => r.ok ? r.json() : [])
      .then((d) => {
  ...
  const compare = useCallback(async () => {
    ...
    try {
      const [snapA, snapB] = await Promise.all([
        fetchSnapshot(transport, nsA, projectId),
        fetchSnapshot(transport, nsB, projectId),
      ]);
