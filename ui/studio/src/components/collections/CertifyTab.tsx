...
async function fetchAudit(transport: Transport, namespace: string, projectId: string): Promise<AuditSnap> {
  const res = await fetch(transport.path(projectId, `/namespace-audit?namespace=${encodeURIComponent(namespace)}`), {
    cache: "no-store",
  });
...
async function buildCertificate(
  ...
  const [audit, blake3] = await Promise.all([
    fetchAudit(transport, namespace, projectId),
    fetchGlobalHash(),
  ]);
...
function CertSection({
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      const h = await fetch(transport.path(projectId, `/health`), { cache: "no-store" }).then((r) =>
        r.ok ? r.json() : {}
      ) as { version?: string };
      const res = await buildCertificate(transport, collection, namespace, h.version ?? "unknown", projectId);
      setResult(res);
    } catch (e) {
  ...
  }, [transport, projectId, collection, namespace]);
...
function TamperSection({ projectId, namespace }: { projectId: string; namespace: string }) {
  ...
  const fetchCurrent = useCallback(async () => {
    ...
    try {
      const [audit, blake3] = await Promise.all([
        fetchAudit(transport, namespace, projectId).catch(() => null),
        fetchGlobalHash(),
      ]);
      setCurrent({
        blake3,
