...
export function MultiSearch({ projectId, namespace, dim, onDelete }: Props) {
  ...
  const run = async () => {
    ...
    try {
      if (mode === "semantic" || mode === "hybrid") {
        ...
        const recRes = await fetch(transport.path(projectId, `/records/${idNum}${qs}`));
        if (!recRes.ok) {
  ...
  const findSimilar = async (id: number) => {
    ...
    try {
      const qs = namespace ? `?collection=${encodeURIComponent(namespace)}` : "";
      const recRes = await fetch(transport.path(projectId, `/records/${id}${qs}`));
      if (!recRes.ok) {
        const body = await recRes.json().catch(() => ({})) as { error?: string };
        throw new Error(body.error ?? `Record not found (${recRes.status})`);
      }
  ...
  const startEdit = async (id: number) => {
    ...
    const qs = namespace ? `?collection=${encodeURIComponent(namespace)}` : "";
    const res = await fetch(transport.path(projectId, `/records/${id}${qs}`)).catch(() => null);
  ...
  const saveMetadata = async (id: number) => {
    ...
    try {
      const qs = namespace ? `?collection=${encodeURIComponent(namespace)}` : "";
      const res = await fetch(transport.path(projectId, `/records/${id}/metadata${qs}`), {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
