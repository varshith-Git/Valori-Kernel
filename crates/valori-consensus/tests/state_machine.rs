...
async fn blank_entries_advance_last_applied_without_touching_state() {
    ...
    assert_eq!(
        sm.state_hash().await,
        before,
        "blank entry must not change state"
    );
    let (last, _) = sm.applied_state().await.unwrap();
    assert_eq!(last, Some(log_id(1, 1, 1)));
}
...
async fn audit_sink_sees_successful_applies_in_order_and_nothing_else() {
    ...
    assert_eq!(
        recorded[0].1,
        Some(rid(1)),
        "request_id travels to the audit record"
    );
}
...
async fn snapshot_roundtrip_preserves_state_hash_and_dedup() {
    ...
    assert_eq!(
        last,
        Some(log_id(1, 1, 3)),
        "last_applied restored from meta"
    );
    ...
    let replies = follower
        ...
        .unwrap();
    assert!(
        replies[0].deduplicated,
        "dedup table survives snapshot transfer"
    );
}
...
async fn corrupted_snapshot_payload_is_refused_and_state_kept() {
    let mut sm = ValoriStateMachine::default();
    sm.apply(vec![normal(1, 1, 1, insert_event(0), None)])
        .await
        .unwrap();
    let before = sm.state_hash().await;
    ...
    let entries = || {
        vec![
            ...
            normal(
                ...
                1,
                3,
                KernelEvent::SoftDeleteRecord { id: RecordId(0) },
                None,
            ),
...
async fn auto_create_namespace_assigns_sequential_ids() {
    ...
    let r1 = sm
        .apply(vec![normal(1, 1, 1, create_ns_event("docs"), None)])
        .await
        .unwrap();
    let r2 = sm
        .apply(vec![normal(1, 1, 2, create_ns_event("images"), None)])
        .await
        .unwrap();
...
async fn auto_create_namespace_is_idempotent_by_name() {
    ...
    let first = sm
        ...
        .unwrap();
    let second = sm
        .apply(vec![normal(1, 1, 2, create_ns_event("docs"), None)])
        .await
        .unwrap();

    assert_eq!(
        first[0].allocated_namespace_id,
        second[0].allocated_namespace_id
    );
...
async fn drop_namespace_removes_from_registry() {
    ...
    sm.apply(vec![normal(1, 1, 1, create_ns_event("docs"), None)])
        .await
        .unwrap();
    assert_eq!(sm.resolve_namespace(Some("docs")).await, Some(1));

    sm.apply(vec![normal(1, 1, 2, drop_ns_event("docs"), None)])
        .await
        .unwrap();
    assert_eq!(
        sm.resolve_namespace(Some("docs")).await,
        None,
        "dropped namespace must no longer resolve"
    );
}
...
async fn drop_unknown_namespace_is_rejected_registry_unchanged() {
    ...
    let replies = sm
        .apply(vec![normal(1, 1, 1, drop_ns_event("never-created"), None)])
        .await
        .unwrap();
    assert!(
        replies[0].rejected.is_some(),
        "dropping an unknown namespace must be rejected"
    );
...
async fn snapshot_roundtrip_preserves_namespace_registry() {
    ...
    follower
        .apply(vec![normal(2, 2, 3, create_ns_event("videos"), None)])
        .await
        .unwrap();
...
async fn two_nodes_applying_the_same_entries_converge_on_namespace_ids() {
    ...
    assert_eq!(
        a.resolve_namespace(Some("docs")).await,
        b.resolve_namespace(Some("docs")).await
    );
    assert_eq!(
        a.resolve_namespace(Some("images")).await,
        b.resolve_namespace(Some("images")).await
    );
    assert_eq!(
        a.state_hash().await,
        b.state_hash().await,
