...
fn main() -> Result<()> {
    println!(
        "🚀 Persistence Benchmark  (real Engine, {} vectors)",
        INGEST_LIMIT
    );
    ...
    let file =
        File::open(path).expect("SIFT base file not found at data/sift/sift/sift_base.fvecs");
    let mmap = unsafe { Mmap::map(&file)? };
    let mut loader =
        SiftBatchLoader::new(&mmap).ok_or_else(|| anyhow::anyhow!("Invalid SIFT fvecs format"))?;
    ...
    'outer: while let Some((raw_bytes, count)) = loader.next_batch(1_000) {
        ...
        for i in 0..count {
            ...
            engine
                .insert_record_from_f32(vec_f32)
    ...
    println!("──────────────────────────────────────────────");
    println!(
        "  PERSISTENCE REPORT  ({} vectors, {} KB snapshot)",
        INGEST_LIMIT,
        snap_bytes.len() / 1024
    );
