...
//! - Event log replay (canonical truth)
...
/// Storage-layer errors returned by recovery operations.
#[derive(Debug, Error)]
...
pub fn replay_wal(
    ...
    wal_path: &Path,
) -> StorageResult<(usize, blake3::Hasher)> {
    let dim = state.dim.map(|d| d as u32);
    let reader = WalReader::open(wal_path, dim)
        .map_err(|e| StorageError::InvalidInput(format!("Failed to open WAL: {}", e)))?;
    ...
    for result in reader {
        let cmd = result
            .map_err(|e| StorageError::InvalidInput(format!("WAL read error: {}", e)))?;

        state.apply(&cmd)
            .map_err(StorageError::Kernel)?;

        let cmd_bytes = bincode::serde::encode_to_vec(&cmd, bincode::config::standard())
             .map_err(|e| StorageError::InvalidInput(format!("Hash serialization failed: {}", e)))?;
...
pub fn recover_from_events(
    event_log_path: &Path
) -> StorageResult<(KernelState, EventJournal, u64)> {
    tracing::info!("Recovering from event log: {:?}", event_log_path);

    recover_from_event_log(event_log_path)
        .map_err(|e| StorageError::InvalidInput(format!("Event replay failed: {:?}", e)))
}
...
pub fn validate_snapshot(
    ...
    let snapshot_data = std::fs::read(snapshot_path)
        .map_err(|e| StorageError::InvalidInput(format!("Failed to read snapshot: {}", e)))?;

    let snapshot_state: KernelState = decode_state(&snapshot_data)
        .map_err(|e| StorageError::InvalidInput(format!("Failed to decode snapshot: {:?}", e)))?;
