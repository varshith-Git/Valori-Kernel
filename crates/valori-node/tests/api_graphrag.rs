...
async fn graphrag_returns_hits_and_connected_subgraph() {
    ...
    let (st, w) = post(
        ...
        serde_json::json!({ "vector": vec_n(0.10) }),
    )
    .await;
    ...
    let (st, _) = post(
        &shared,
        "/graph/edge",
        serde_json::json!({ "from": chunk, "to": doc, "kind": 0 }),
    )
    .await;
    assert_eq!(st, StatusCode::OK);
    ...
    post(
        &shared,
        "/v1/memory/upsert_vector",
        serde_json::json!({ "vector": vec_n(0.5) }),
    )
    .await;
    post(
        &shared,
        "/v1/memory/upsert_vector",
        serde_json::json!({ "vector": vec_n(0.9) }),
    )
    .await;
    ...
    let (st, out) = post(
        &shared,
        "/v1/graphrag",
        serde_json::json!({ "query_vector": vec_n(0.10), "k": 3, "depth": 2 }),
    )
    .await;
    assert_eq!(st, StatusCode::OK);
...
async fn graphrag_on_empty_store_is_empty_not_error() {
    ...
    let (st, out) = post(
        &shared,
        "/v1/graphrag",
        serde_json::json!({ "query_vector": vec_n(0.1), "k": 5 }),
    )
    .await;
    assert_eq!(st, StatusCode::OK);
...
async fn graphrag_depth_zero_returns_seeds_without_edges() {
    ...
    let (_, w) = post(
        &shared,
        "/v1/memory/upsert_vector",
        serde_json::json!({ "vector": vec_n(0.2) }),
    )
    .await;
    let chunk = w["chunk_node_id"].as_u64().unwrap();
    let doc = w["document_node_id"].as_u64().unwrap();
    ...
    let (st, out) = post(
        &shared,
        "/v1/graphrag",
        serde_json::json!({ "query_vector": vec_n(0.2), "k": 1, "depth": 0 }),
    )
    .await;
    assert_eq!(st, StatusCode::OK);
