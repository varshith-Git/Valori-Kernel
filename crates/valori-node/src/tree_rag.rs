...
//!
//! Two ideas, ported from the `valori_tree_rag` spike into the product:
...
//!
//! ...plus the two things plain vector search does not give you, which are the
//! Valori moat:
...
//! ## Determinism
//!
//! Unlike the spike, this Rust port has **no optional LLM hook** — the navigator
//! is purely deterministic term-frequency reasoning over the tree. That is a
//! feature, not a limitation: tree retrieval here is reproducible bit-for-bit,
//! matching Valori's whole ethos. (An LLM reasoner, if ever added, would live
//! here in `valori-node`, never in the `no_std` kernel.)
//!
//! ## Statelessness
//!
//! Build / query / verify are pure functions over the document text and the
//! serialized tree — no engine state, no kernel mutation. This is why the HTTP
//! handlers drop unchanged into both the standalone and cluster routers.
...
// ── BLAKE3 helpers (same hash family as the kernel audit chain) ───────────────
...
/// Hash of several parts joined by the ASCII unit-separator (0x1f).
fn join_hash(parts: &[&str]) -> String {
    blake3::hash(parts.join("\u{1f}").as_bytes()).to_hex().to_string()
}

// ── stopwords for the deterministic navigator ─────────────────────────────────
...
/// Lowercase alphanumeric tokens, dropping single-char tokens and stopwords.
...
/// A compact table-of-contents entry (title + summary, no body) that the
/// navigator reasons over.
#[derive(Clone, Debug, Serialize, Deserialize)]
...
impl TreeIndex {
    ...
    pub fn from_markdown(text: &str, doc_name: &str) -> Self {
        ...
        // (line_num 1-indexed, level, title)
        ...
        let mut stack: Vec<(String, usize)> = Vec::new(); // (node_id, level)
        ...
        for idx in 0..headers.len() {
            ...
            let body_start = line_num; // skip the header line itself
    ...
    /// sorted best-first. Title matches weigh most, then summary, then body — the
    /// reasoning a human does scanning a table of contents.
    ...
    pub fn answer(&self, query: &str, k: usize, prev_hash: &str) -> AnswerResult {
        ...
        // Offline / deterministic: the honest vectorless answer is the verbatim
        // section(s). (A prose-composing LLM would slot in here, off-kernel.)
...
/// The full result of a tree-navigated answer.
#[derive(Clone, Debug, Serialize, Deserialize)]
...
/// One tamper-evident record of a single retrieval. Seals the previous
/// receipt's hash into `receipt_hash`, forming a chain — the off-kernel shadow
/// of the kernel's `events.log`.
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct Receipt {
    pub query: String,
...
impl Receipt {
    ...
    /// Recompute this receipt's hash from its fields and check it seals
    /// `prev_hash`. Used by `verify_chain`.
...
/// Verify that a sequence of receipts forms an unbroken chain — every link
/// seals the one before it, none removed or reordered.
...
fn parse_header(s: &str) -> Option<(usize, String)> {
    ...
    // require a space after the hashes (markdown atx headers)
...
fn first_sentence(text: &str) -> Option<String> {
    ...
    // first run up to a sentence terminator
...
/// Build response now includes `cache_key` — a BLAKE3 fingerprint of the input
/// text. Pass it to `/v1/tree/query` instead of re-transmitting the full tree.
#[derive(Serialize)]
...
pub struct QueryRequest {
    /// Full tree (backward-compat). Pass either this OR `cache_key`.
    #[serde(default)]
    pub tree: Option<TreeIndex>,
    /// Key returned by `/v1/tree/build`. The server looks the tree up from its
    /// in-process cache — avoids re-transmitting the full tree on every call.
    #[serde(default)]
...
pub struct HybridHit {
    /// `"tree"` or `"vector"`.
    pub source: String,
    /// Combined normalised score in \[0, 1\].
    pub score: f64,
    // tree fields — Some for tree hits, None for vector-only hits
    ...
    // vector fields — Some for vector hits, None for tree-only hits
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct HybridRequest {
    /// Raw markdown text to build a tree from, OR…
    #[serde(default)]
    pub text: Option<String>,
    /// …a pre-built tree, OR…
    #[serde(default)]
    pub tree: Option<TreeIndex>,
    /// …a cache_key returned by a previous `/v1/tree/build` call.
    #[serde(default)]
    pub cache_key: Option<String>,
    pub query: String,
    /// Namespace to search for vector hits. Defaults to `"default"`.
    #[serde(default)]
    pub namespace: Option<String>,
    #[serde(default = "default_k")]
    pub k: usize,
    /// How much weight to give tree scores vs vector scores \[0, 1\].
    /// `0.6` means 60 % tree, 40 % vector. Default: 0.6.
    #[serde(default = "default_tree_weight")]
    pub tree_weight: f64,
    #[serde(default)]
    pub prev_hash: Option<String>,
    /// Name to use when building a tree from `text`. Ignored if `tree` is provided.
    #[serde(default)]
    pub doc_name: Option<String>,
}
...
pub struct HybridResponse {
    ...
    /// Structured tree answer (citations + receipt) for the top tree hits.
...
impl TreeIndex {
    ...
    /// Used by the hybrid endpoint to combine with normalised vector scores.
...
/// Stateless: no engine needed; the caller re-sends the tree + receipt.
...
/// an unbroken BLAKE3 chain (each receipt's `prev_hash` matches the previous
/// receipt's `receipt_hash`, starting from the genesis constant).
/// Stateless: no engine needed; the caller supplies the receipts array.
...
mod tests {
    ...
    fn tampering_is_caught_by_receipt_replay() {
        ...
        // Tamper: alter the stored section the receipt points at.
        ...
        assert!(!tampered.verify_receipt(&r.receipt), "tampering must be detected");
    }
