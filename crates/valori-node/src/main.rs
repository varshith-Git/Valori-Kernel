...
async fn main() {
    ...
    if let Some(root) = std::env::var_os("VALORI_SHARED_ROOT") {
        assert!(
            std::env::var_os("VALORI_CLUSTER_MEMBERS").is_none(),
            "shared hosting cannot run in Raft mode"
        );
        let token = std::env::var("VALORI_SHARED_ADMIN_TOKEN")
            .expect("VALORI_SHARED_ADMIN_TOKEN is required");
        let capacity = std::env::var("VALORI_SHARED_MAX_PROJECTS")
            .unwrap_or_else(|_| "100".into())
            .parse()
            .expect("VALORI_SHARED_MAX_PROJECTS must be a positive integer");
        let host =
            valori_node::shared::SharedHost::open(std::path::Path::new(&root), &token, capacity)
        ...
        let listener = TcpListener::bind(&bind)
            .await
            .expect("cannot bind shared node");
