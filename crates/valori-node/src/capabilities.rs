    ...
    async fn save_snapshot(&self, _shard_id: u8, path: Option<&str>) -> Result<String, EffectError> {
        ...
        let (target, data, hash) = {
            ...
            let target = path.map(std::path::PathBuf::from)
                ...
                .ok_or_else(|| EffectError::Dispatch("snapshot: No snapshot path configured".into()))?;
            let data = eng.snapshot()
                .map_err(|e| EffectError::Dispatch(format!("snapshot encode: {e}")))?;
            let hash: String = hash_state_blake3(&eng.state).iter().map(|b| format!("{:02x}", b)).collect();
            (target, data, hash)
        ...
        let mut results: Vec<serde_json::Value> = hits.iter().map(|(record_id, score)| {
            ...
            let (decay_factor, age_secs) = match decay_half_life_secs {
                Some(hl) if hl > 0.0 => {
                    ...
                    (Some(valori_search::decay_factor(age_s, hl as u64)), Some(age_s))
                }
                _ => (None, None),
            };
            serde_json::json!({
...
impl RaftKernelCapability {
    pub fn new(
        ...
        RaftKernelCapability { shards, sm, shard_count, tree_cache, community_store, embed_config, http }
    }
}
...
impl KernelCapability for RaftKernelCapability {
            ...
            tokio::task::block_in_place(|| {
                tokio::runtime::Handle::current().block_on(async move {
                    sm.with_state(|kernel| {
                        hash_state_blake3(kernel).iter().map(|b| format!("{b:02x}")).collect::<String>()
                    }).await
                })
            })
        } else {
    ...
    async fn save_snapshot(&self, shard_id: u8, _path: Option<&str>) -> Result<String, EffectError> {
        use valori_consensus::types::ShardId;
        use valori_kernel::snapshot::blake3::hash_state_blake3;
        let sid = ShardId(shard_id as u32);
        let shard = self.shards.get(&sid)
        ...
        let hash: String = hash_state_blake3(&kernel_state).iter().map(|b| format!("{:02x}", b)).collect();
        Ok(hash)
    }
    ...
    async fn graph_rag(
        ...
        let fxp_data: Result<Vec<FxpScalar>, EffectError> = vector.iter().map(|&v| {
            if v > 32767.99 || v < -32768.0 {
                Err(EffectError::TaskFailed("query vector value out of Q16.16 range".into()))
            } else {
                Ok(FxpScalar((v * SCALE as f32) as i32))
            }
        }).collect();
        ...
        let (raw_hits, seeds, nodes, edges): (Vec<(u32, f32, Option<u32>)>, Vec<u32>, _, _) =
            shard.state_machine.with_state(move |s| {
                let mut buf = vec![SearchResult::default(); k_usize];
                let n = s.search_l2_ns(&fxp_q, &mut buf, namespace_id);
                let hits: Vec<(u32, f32)> = buf[..n].iter().map(|r| {
                ...
                let mut seeds: Vec<u32> = Vec::new();
                let raw: Vec<(u32, f32, Option<u32>)> = hits.iter().map(|(record_id, score)| {
        ...
        Ok(serde_json::json!({ "hits": hits_out, "seed_nodes": seeds, "subgraph": { "nodes": nodes, "edges": edges } }))
    }
    ...
    async fn memory_search(
        ...
        let over_k = if rerank || metadata_filter.is_some() { (k as usize) * 10 } else { k as usize };
        let fxp_data: Result<Vec<FxpScalar>, EffectError> = vector.iter().map(|&v| {
            if v > 32767.99 || v < -32768.0 {
                Err(EffectError::TaskFailed("query vector value out of Q16.16 range".into()))
            } else {
        ...
        let raw: Vec<(u32, f32, Option<u64>)> = shard.state_machine
        ...
        if rerank {
            if let Some(qt) = query_text.as_deref() {
                let raw_candidates: Vec<(u64, f32)> = candidates.iter()
                    .map(|c| (c.record_id as u64, c.score))
                    .collect();
                let reranked = shard.state_machine.with_text_corpus(|corpus| {
                ...
                candidates.sort_by_key(|c| {
                    order.iter().position(|&id| id == c.record_id as u64).unwrap_or(usize::MAX)
                });
            }
        }
    ...
    async fn community_detect(
        ...
        let ns_filter = if namespace_id == 0 { None } else { Some(namespace_id) };

        let store = shard.state_machine.with_state(move |kernel| {
        ...
        let communities: Vec<valori_rag::community::CommunitySummary> = store.members.iter()
        ...
        *self.community_store.write().await = Some(store);
        ...
        serde_json::to_value(&resp).map_err(|e| EffectError::Dispatch(format!("community_detect serialize: {e}")))
    }
    ...
    async fn community_search(
        ...
        let store = store_guard.as_ref()
            .ok_or_else(|| EffectError::Dispatch("community index not built — call POST /v1/community/detect first".into()))?;

        let ranked = valori_rag::community::rank_communities(store, &vector, k as usize);
        let total = store.centroids.len();
        let communities: Vec<valori_rag::community::CommunityHit> = ranked.into_iter()
            .map(|(cid, score)| {
                let members = store.members.get(&cid).map(|v| v.as_slice()).unwrap_or(&[]);
        ...
        let resp = valori_rag::community::SearchResponse { communities, total_communities_searched: total };
        serde_json::to_value(&resp).map_err(|e| EffectError::Dispatch(format!("community_search serialize: {e}")))
    }

    async fn tree_build(&self, text: String, doc_name: String) -> Result<serde_json::Value, EffectError> {
        let tree = valori_rag::tree::TreeIndex::from_markdown(&text, &doc_name);
        let cache_key = valori_rag::tree::hash_text(&text);
        self.tree_cache.write().await.insert(cache_key.clone(), tree.clone());
    ...
    async fn tree_query(
        ...
        let tree: valori_rag::tree::TreeIndex = if let Some(key) = tree_json.as_str() {
            self.tree_cache.read().await.get(key).cloned()
                .ok_or_else(|| EffectError::Dispatch(format!("tree not in cache: {key}")))?
        } else {
            serde_json::from_value(tree_json)
        ...
        let prev = prev_hash.as_deref().unwrap_or(valori_rag::tree::GENESIS);
        let result = tree.answer(&query, k.max(1) as usize, prev);
        serde_json::to_value(&result).map_err(|e| EffectError::Dispatch(format!("tree_query serialize: {e}")))
    }
    ...
    async fn tree_hybrid(
        ...
        let cache_key = params.get("cache_key").and_then(|v| v.as_str()).map(|s| s.to_string());
        let tree_weight = params.get("tree_weight").and_then(|v| v.as_f64()).unwrap_or(0.5) as f32;
        let prev = params.get("prev_hash").and_then(|v| v.as_str()).unwrap_or(GENESIS).to_string();
        ...
        if vw > 0.0 {
            ...
            if !query_vec.is_empty() {
                let sid = ShardId(shard_id as u32);
                if let Some(shard) = self.shards.get(&sid) {
                    let fxp_data: Vec<FxpScalar> = query_vec.iter()
                        .map(|&v| FxpScalar((v * SCALE as f32) as i32))
                        .collect();
                    let fxp_q = FxpVector { data: fxp_data };
                    let fetch = k_usize * 2;
                    let raw_hits: Vec<(u32, f32)> = shard.state_machine.with_state(move |kernel| {
        ...
        let reasoning = format!("{} tree hits, {} vector hits{}", tree_hit_count, vector_hit_count, reasoning_extra);
        let resp = HybridResponse { query, hits, tree_hit_count, vector_hit_count, tree_answer, reasoning };
        serde_json::to_value(&resp).map_err(|e| EffectError::Dispatch(format!("tree_hybrid serialize: {e}")))
    }
}
...
impl CapabilityRegistryBuilder {
    ...
    pub fn build_cluster(
        ...
        let kernel: Arc<dyn KernelCapability> = Arc::new(
            RaftKernelCapability::new(shards, sm, shard_count, tree_cache, community_store, embed_config.clone(), http_client.clone())
        );
