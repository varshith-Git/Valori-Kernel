...
impl Reader for DocxReader {
    ...
    async fn read(&self, input: &str, source: Option<&str>) -> Result<Document, IngestError> {
        ...
        let (text, title, author) = tokio::task::spawn_blocking(move || extract_docx(&path_owned))
            .await
...
fn extract_docx(
    ...
    let file = std::fs::File::open(path).map_err(|e| IngestError::Reader(e.to_string()))?;
    let mut zip = zip::ZipArchive::new(file).map_err(|e| IngestError::Reader(e.to_string()))?;

    let body_text = read_entry_xml(&mut zip, "word/document.xml", extract_text_runs)?;
    let (title, author) =
        read_entry_xml(&mut zip, "docProps/core.xml", extract_core_props).unwrap_or((None, None));
...
mod tests {
    ...
    async fn missing_file_returns_reader_error() {
        let err = DocxReader
            .read("/nonexistent/doc.docx", None)
            .await
            .unwrap_err();
