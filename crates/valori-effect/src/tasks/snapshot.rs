...
impl Task for SnapshotArtifactTask {
    fn name(&self) -> &'static str { "snapshot_artifact" }
    ...
    async fn run(
        ...
        let inputs: SnapshotInputs = serde_json::from_str(inputs_json)
            .map_err(|e| EffectError::TaskFailed(format!("SnapshotArtifactTask bad inputs: {e}")))?;

        let state_hash = ctx.capabilities.kernel
        ...
        let _ = ctx.bus.dispatch(Effect::ephemeral(
            metric_id,
            EffectPayload::Counter { name: "snapshots_saved".into(), value: 1.0 },
        )).await;
