...
fn default_doc_name() -> String { "document".into() }

pub struct TreeBuildTask;

#[async_trait]
impl Task for TreeBuildTask {
    fn name(&self) -> &'static str { "tree_build" }
    ...
    async fn run(
        ...
        let _ = ctx.bus.dispatch(Effect::ephemeral(
            metric_id,
            EffectPayload::Counter { name: "tree_builds".into(), value: 1.0 },
        )).await;
...
impl Task for TreeQueryTask {
    fn name(&self) -> &'static str { "tree_query" }
    ...
    async fn run(
        ...
        let _ = ctx.bus.dispatch(Effect::ephemeral(
            metric_id,
            EffectPayload::Counter { name: "tree_queries".into(), value: 1.0 },
        )).await;
...
impl Task for TreeHybridTask {
    fn name(&self) -> &'static str { "tree_hybrid" }
    ...
    async fn run(
        ...
        let result = ctx.capabilities.kernel
            .tree_hybrid(inputs.shard_id, inputs.namespace_id, inputs.query, inputs.k, inputs.params)
            .await?;

        let metric_id = EffectId::new(&ctx.execution_id, ctx.topological_index, 0);
        let _ = ctx.bus.dispatch(Effect::ephemeral(
            metric_id,
            EffectPayload::Counter { name: "tree_hybrid_queries".into(), value: 1.0 },
        )).await;
